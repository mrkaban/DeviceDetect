# -*- coding: utf-8 -*-
"""
Network Scanner - PySide6 приложение для сканирования сети
...
from PySide6.QtWidgets import (QApplication, QMainWindow, QLabel, QLineEdit, QPushButton,
                             ...
                             QHBoxLayout, QCheckBox, QMenuBar, QMenu, QMessageBox,
                             QInputDialog)
...
class MyWindow(QMainWindow):
    ...
    def _show_about(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("О программе DeviceDetect")
        dialog.setMinimumWidth(500)
        layout = QVBoxLayout()
        
        info_text = """
        <h2>DeviceDetect</h2>
        <p><b>Описание:</b> Программа для обнаружения новых устройств в сети с отправкой отчетов на указанные адреса электронной почты. Отслеживает изменения IP, MAC, Hostname. Для работы требуется установленная NMAP.</p>
        <p><b>Лицензия:</b> GNU GPL v3</p>
        <p><b>Язык:</b> Русский</p>
        <p><b>Разработчик:</b> Алексей Черемных</p>
        <p><b>Версия:</b> 2.0</p>
        <p><b>Официальный сайт:</b> <a href="https://alekseycheremnykh.ru/post/device-detect/">https://alekseycheremnykh.ru/post/device-detect/</a></p>
        """
    ...
    def _open_history_viewer(self):
        ...
        from PySide6.QtWidgets import QDialog, QVBoxLayout, QTextEdit, QPushButton, QHBoxLayout, QLineEdit, QLabel
    ...
    def deep_scan_ip(self, ip, nmap_dir):
        ...
        # Быстрая проверка доступности хоста (ping) с анализом вывода
        try:
            ping_result = subprocess.run(['ping', '-n', '1', '-w', '500', ip],
                                         stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                         shell=False, timeout=3, encoding='cp1251', errors='replace')
            output = ping_result.stdout.lower()
            # Критерии недоступности:
            # 1. Код возврата не ноль (потеря всех пакетов)
            # 2. В выводе есть "превышен срок жизни" (TTL expired) - ответ от другого устройства
            # 3. В выводе есть "заданный узел недоступен" или "destination host unreachable"
            # 4. В выводе нет строки "ответ от" (для русского) или "reply from" (англ.)
            if ping_result.returncode != 0:
                print(f"  deep_scan: хост {ip} недоступен (код возврата {ping_result.returncode})")
                return {'hostname': 'None', 'mac': 'None', 'ping_ok': False}
            if 'превышен срок жизни' in output or 'ttl expired' in output:
                print(f"  deep_scan: хост {ip} недоступен (TTL превышен)")
                return {'hostname': 'None', 'mac': 'None', 'ping_ok': False}
            if 'заданный узел недоступен' in output or 'destination host unreachable' in output:
                print(f"  deep_scan: хост {ip} недоступен (узел недоступен)")
                return {'hostname': 'None', 'mac': 'None', 'ping_ok': False}
            # Проверяем, что есть ответ от целевого IP (необязательно, но можно)
            if 'ответ от' not in output and 'reply from' not in output:
                print(f"  deep_scan: хост {ip} недоступен (нет ответа)")
                return {'hostname': 'None', 'mac': 'None', 'ping_ok': False}
        except Exception as e:
            print(f"  deep_scan: ping {ip} ошибка: {e}")
            # В случае ошибки выполнения ping считаем хост недоступным
            return {'hostname': 'None', 'mac': 'None', 'ping_ok': False}
